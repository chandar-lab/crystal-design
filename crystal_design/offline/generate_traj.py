...
def evaluate_test(data_path):
    ...
    for batch in tqdm(data):
        ...
        j = 0
        atomic_number = deepcopy(batch.ndata['atomic_number'])
        position = deepcopy(batch.ndata['position'])
        lengths = deepcopy(batch.lengths_angles_focus.cpu()[0][:3])
        angles = deepcopy(batch.lengths_angles_focus.cpu()[0][3:6])
...
def generate_trajectories_tensor(file_name, save_path, metals_prop, nonmetals_prop, eps = 0.):
    ...
    # s = list(list(nonmetals_prop.keys()))
    # s = [3696]
    n_samples = data.shape[0]
    j = 0
    for i in tqdm(s):
        ...
        if prop[0] >= 0.5 and prop[0] <= 1.5 or prop[0] >= 3.5 and prop [0]<= 4.5:
...
if __name__ == '__main__':
    ...
    generate_trajectories_tensor(file_name = '/home/mila/p/prashant.govindarajan/scratch/crystal_design_project/crystal-design/crystal_design/data/mp_20/train.csv',
                                save_path = '/home/mila/p/prashant.govindarajan/scratch/crystal_design_project/crystal-design/crystal_design/offline/trajectories/train_desired_range.pt',
                                metals_prop = metals_prop,
                                nonmetals_prop = nonmetals_prop)


    ## EVALUATION
    # data_path = '../runner/val_generated/val_nm_x1_112.pt'
    # print(data_path)
    # evaluate_test(data_path)
