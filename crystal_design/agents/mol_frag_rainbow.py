...
from crystal_design.agents.dqn import DQNGraphAgent
from crystal_design.agents.qnets.egnn import EGNNetwork


class RainbowDQNGraphTransformerMolFragAgent(DQNGraphAgent):
    ...
    def create_q_networks(self):
        ...
        network = GraphTransformerFragGFN(
            self.env_ctx,
            self.model_num_emb,
    ...
    def act(self, observation):
        ...
        qvals, graph_action_cat = self._qnet(observation)
        if self._rng.random() < epsilon:
            probs = np.ones(self._action_space.n)
            if graph_action_cat.masks is not None:
                mask = torch.cat(graph_action_cat.masks)
                probs *= mask

            probs = probs / probs.sum()
            action = self._rng.choice(np.arange(self._action_space.n), p=probs)
        else:
    ...
    def update(self, update_info):
        ...
        info = self.preprocess_update_info(update_info)
        info['observation'] = info['observation'].cpu().numpy()
        self._replay_buffer.add(**info)
